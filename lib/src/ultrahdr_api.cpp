...
uhdr_error_info_t uhdr_enc_set_raw_image(uhdr_codec_private_t* enc, uhdr_raw_image_t* img,
  ...
  if (dynamic_cast<uhdr_encoder_private*>(enc) == nullptr) {
    ...
    status.has_detail = 1;
    snprintf(status.detail, sizeof status.detail,
             "invalid input pixel format %d, expects one of {UHDR_IMG_FMT_24bppYCbCrP010, "
  ...
  std::unique_ptr<ultrahdr::uhdr_raw_image_ext_t> entry = ultrahdr::convert_input_to_YCbCr(img);
  if (entry == nullptr) {
    status.error_code = UHDR_CODEC_UNKNOWN_ERROR;
    status.has_detail = 1;
    snprintf(status.detail, sizeof status.detail,
