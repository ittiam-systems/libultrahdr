...
using TestParamsEncoderAPI0 =
    std::tuple<std::string, int, int, uhdr_color_gamut_t, uhdr_color_transfer_t, int, int, float>;
using TestParamsEncoderAPI1 =
    std::tuple<std::string, std::string, int, int, uhdr_color_gamut_t, uhdr_color_transfer_t,
               uhdr_color_gamut_t, int, int, float, uhdr_enc_preset_t>;
...
static bool loadFile(const char* filename, void*& result, int length) {
  ...
  if (ifd.good()) {
    int size = ifd.tellg();
    if (size < length) {
      ALOGE("Requested to read %zu bytes from file: %s, file contains only %zu bytes", length,
            filename, size);
      return false;
    }
...
class DecBenchmark {
 public:
  ...
  ~DecBenchmark() {
    if (mUhdrImg.data) free(mUhdrImg.data);
  }
...
class EncBenchmark {
 public:
  ...
  EncBenchmark(TestParamsEncoderAPI0 testParams) {
    ...
    mMapDimensionScaleFactor = std::get<6>(testParams);
    mGamma = std::get<7>(testParams);
  };
  ...
  EncBenchmark(TestParamsEncoderAPI1 testParams) {
    ...
    mMapDimensionScaleFactor = std::get<8>(testParams);
    mGamma = std::get<9>(testParams);
    mEncPreset = std::get<10>(testParams);
  }
...
static void BM_UHDREncode_Api0(benchmark::State& s, TestParamsEncoderAPI0 testVectors) {
  ...
  s.SetLabel(benchmark.mHdrFile + ", " + std::to_string(benchmark.mWidth) + "x" +
...
static void BM_UHDREncode_Api1(benchmark::State& s, TestParamsEncoderAPI1 testVectors) {
  ...
  s.SetLabel(benchmark.mHdrFile + ", " + std::to_string(benchmark.mWidth) + "x" +
             std::to_string(benchmark.mHeight) + ", " + colorGamutToString(benchmark.mHdrCg) +
             ", " + tfToString(benchmark.mHdrCt) + ", Sdr: " + benchmark.mSdrFile + ", " +
             colorGamutToString(benchmark.mSdrCg) + ", " + tfToString(benchmark.mSdrCt));
...
void addTestVectors() {
  ...
  testParamsDecodeAPI.push_back(
      {"mountains.jpg", UHDR_CT_HLG, UHDR_IMG_FMT_32bppRGBA1010102, false});
  testParamsDecodeAPI.push_back(
      {"mountains.jpg", UHDR_CT_PQ, UHDR_IMG_FMT_32bppRGBA1010102, false});
  testParamsDecodeAPI.push_back(
      {"mountains.jpg", UHDR_CT_LINEAR, UHDR_IMG_FMT_64bppRGBAHalfFloat, false});
  testParamsDecodeAPI.push_back(
      {"mountains.jpg", UHDR_CT_HLG, UHDR_IMG_FMT_32bppRGBA1010102, true});
  testParamsDecodeAPI.push_back({"mountains.jpg", UHDR_CT_PQ, UHDR_IMG_FMT_32bppRGBA1010102, true});
  testParamsDecodeAPI.push_back(
      {"mountains.jpg", UHDR_CT_LINEAR, UHDR_IMG_FMT_64bppRGBAHalfFloat, true});
  testParamsDecodeAPI.push_back({"mountains.jpg", UHDR_CT_SRGB, UHDR_IMG_FMT_32bppRGBA8888, false});

  for (const auto& hdrFile : kEncodeApi0TestImages12MpName) {
    /* Encode API 0 - hdrFile, width, height, hdrColorGamut, hdrColorTransfer,
       useMultiChannelGainmap, gainmapScaleFactor, gamma */
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 0, 4, 1.0f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 1, 4, 1.0f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 0, 1, 1.0f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 0, 4, 1.571f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 1, 1, 1.0f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 1, 4, 1.616f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 0, 1, 1.133f});
    testParamsAPI0.push_back({hdrFile, 4080, 3072, UHDR_CG_BT_2100, UHDR_CT_PQ, 1, 1, 1.6147f});
  }
  ...
  for (const auto& inputFiles : kEncodeApi1TestImages12MpName) {
    /* Encode API 1 - hdrFile, sdrFile, width, height, hdrColorGamut, hdrColorTransfer,
       sdrColorGamut, useMultiChannelGainmap, gainmapScaleFactor, gamma, encPreset */
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 4, 1.0f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 4, 1.0f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 1, 1.0f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 4, 1.571f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 4, 1.0f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 1, 1.0f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 4, 1.616f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 4, 1.0f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 1, 1.133f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 1, 1.0f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 4, 1.571f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 1, 1.6147f, UHDR_USAGE_REALTIME});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 1, 1.0f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 4, 1.0f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 0, 1, 1.133f, UHDR_USAGE_BEST_QUALITY});
    testParamsAPI1.push_back({inputFiles.first, inputFiles.second, 4080, 3072, UHDR_CG_BT_2100,
                              UHDR_CT_PQ, UHDR_CG_BT_709, 1, 1, 1.6147f, UHDR_USAGE_BEST_QUALITY});
  }
}
